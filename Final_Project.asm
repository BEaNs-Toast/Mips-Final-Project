...
wait_choice:
    # spin until a key arrives
    lw   $t9, KEY_PRESS
    lw   $t8, 0($t9)
    beq  $t8, 0, wait_choice
...
spawn_check:
    ...
    la    $t6, DIS_ARR
    add   $t6, $t6, $t4         # address
    lb    $t8, 0($t6)         
    bnez  $t8, game_over      

    addi  $t3, $t3, 4           # next of the 4 cells
    addi  $t5, $t5, 1
    blt   $t5, 4, spawn_check  # loop 4 times

    #draw next piece if no overlap
    setcolor_reg($s5)
    li    $s4, 0
    jal   draw_block
